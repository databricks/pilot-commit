"""
Preprocess the OlympiadBench dataset to parquet format
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_name", type=str, default="oylmpiad-bench-nosystem-boxed.parquet")
    ...
    print(f"Loading OlympiadBench dataset from {data_source}...", flush=True)
    ...
    def convert_data_format(example, idx):
        raw_problem = example['question']
        answer = str(example['final_answer'][0])
        original_id = str(example['id'])

        data = {
            'data_source': f'oylmpiad_bench',
            'prompt': [
                {'content': '', 'role': 'system'},
