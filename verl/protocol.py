...
class DataProto:
    ...
    def reduce(
        ...
        """
        Reduces the DataProto by removing duplicate rows based on specified keys.
        This function is the inverse of `repeat`.

        Args:
            reduce_keys (list[str]): A list of keys (from either `batch`
                or `non_tensor_batch`) to use for identifying duplicates.
            key_op (str): Specifies how to handle the data for keys that are not
                in `reduce_keys` when a duplicate is found.
                - "first": Keeps the value from the first occurrence of the duplicate row.
                - "last": Keeps the value from the last occurrence of the duplicate row.

        Returns:
            DataProto: A new DataProto with duplicate rows removed.

        Raises:
            ValueError: If `reduce_keys` is empty or if `key_op` is not "first" or "last".
            KeyError: If a key in `reduce_keys` is not found in `batch` or
                `non_tensor_batch`.

        Example:
            >>> # Assuming 'data_proto' has a 'state' key with repeated values
            >>> reduced_data = data_proto.reduce(reduce_keys=['state'])

            >>> # Reduces the entire row based on 'action' and 'reward' uniqueness
            >>> reduced_data = data_proto.reduce(reduce_keys=['action', 'reward'])
        """
        if not reduce_keys:
            raise ValueError("`reduce_keys` cannot be an empty list.")

        if key_op not in ["first", "last"]:
            raise ValueError(f"key_op must be 'first' or 'last', but got '{key_op}'")

        if len(self) == 0:
            return type(self)(batch=None, non_tensor_batch={}, meta_info=self.meta_info)

        batch_keys_list = list(self.batch.keys())
        all_keys = batch_keys_list + list(self.non_tensor_batch.keys())

        if not all(key in all_keys for key in reduce_keys):
            raise KeyError(
                f"One or more keys in `reduce_keys` not found in DataProto. "
                f"Found keys: {all_keys}"
            )

        unique_signatures = {} # Use a dictionary to store unique signatures and their indices
        selected_indices = []

        # Iterate through each row of the DataProto
        for i in range(len(self)):
            row_signature = []
            for key in reduce_keys:
                if key in self.batch:
                    # Get the tensor row, flatten it, and convert to a tuple for hashing
                    row_data = self.batch.get(key)[i].flatten().tolist()
                else:
                    # Get the non-tensor row data
                    row_data = self.non_tensor_batch.get(key)[i]
                    # Handle different data types by converting to a hashable format
                    if isinstance(row_data, np.ndarray):
                         row_data = row_data.flatten().tolist()
                    # If it's already a list, tuple, etc., leave it as is
                
                # Convert to a tuple to ensure it's hashable
                row_signature.append(tuple(row_data))
            
            # The full signature of the row across all reduce_keys
            row_signature = tuple(row_signature)
            
            # Check if this signature is already in our unique_signatures
            if row_signature not in unique_signatures:
                # If not, it's a new unique row. Store its index.
                unique_signatures[row_signature] = i
            else:
                if key_op == "last":
                    # If key_op is 'last', update the index to the latest occurrence
                    unique_signatures[row_signature] = i

        # Extract the indices from the dictionary.
        # This will contain either the first or last index for each unique signature.
        selected_indices = list(unique_signatures.values())
        
        # Sort the indices to ensure the final DataProto is in the correct order
        selected_indices.sort()

        return self.select_idxs(torch.tensor(selected_indices, dtype=torch.long))
    ...
    def filter(self, filter_fn: Callable) -> "DataProto":
        """Filter the DataProto based on the filter function.
        
        Args:
            filter_fn (Callable): A function that takes a DataProtoItem and returns a boolean.
                                 Items for which filter_fn returns True will be kept.
        
        Returns:
            DataProto: A new DataProto containing only the filtered items.
        
        Example:
            >>> # Filter items where index is in pass_commit_batch_indices
            >>> filtered = data.filter(lambda item: item.non_tensor_batch["index"] in pass_commit_batch_indices)
        """
        selected_indices = []
        for i in range(len(self)):
            item = self[i]  # Returns DataProtoItem
            if filter_fn(item):
                selected_indices.append(i)
        
        if len(selected_indices) == 0:
            # Return empty DataProto with same structure
            empty_batch = None
            if self.batch is not None:
                empty_batch = TensorDict(
                    source={key: tensor[:0] for key, tensor in self.batch.items()},
                    batch_size=(0,),
                    device=self.batch.device,
                )
            empty_non_tensor = {key: val[:0] for key, val in self.non_tensor_batch.items()}
            return type(self)(batch=empty_batch, non_tensor_batch=empty_non_tensor, meta_info=self.meta_info)
        
        return self.select_idxs(selected_indices)
