...
class Tracking:
    ...
    def finish(self, exit_code=0, quiet=False):
        """Explicitly finish all logging backends.
        
        This method should be called at the end of training to ensure all data is properly
        logged and synced. It includes retry logic for wandb to handle transient network issues.
        
        Args:
            exit_code: Exit code to pass to wandb/vemlp_wandb finish (0 for success)
            quiet: If True, suppress error messages (useful for __del__ fallback)
        """
        import time
        ...
        if self._finished:
            if not quiet:
                print("Logger already finished, skipping...")
            return
        
        # Finish wandb with retry logic for better reliability
        if "wandb" in self.logger:
            ...
            for attempt in range(max_retries):
                try:
                    if not quiet:
                        print(f"Finishing wandb run (attempt {attempt + 1}/{max_retries})...")
                    self.logger["wandb"].finish(exit_code=exit_code, quiet=quiet)
                    if not quiet:
                    ...
                    if attempt < max_retries - 1:
                        if not quiet:
                            print(f"Failed to finish wandb (attempt {attempt + 1}): {e}. Retrying in {retry_delay}s...")
                        time.sleep(retry_delay)
                    else:
                        if not quiet:
                            print(f"Failed to finish wandb after {max_retries} attempts: {e}")
                            print("Some data may not have been synced to wandb servers.")
        
        # Finish other backends
        if "swanlab" in self.logger:
            try:
                self.logger["swanlab"].finish()
            except Exception as e:
                if not quiet:
                    print(f"Error finishing swanlab: {e}")
        ...
        if "vemlp_wandb" in self.logger:
            try:
                ...
                if not quiet:
                    print(f"Error finishing vemlp_wandb: {e}")
        ...
        if "tensorboard" in self.logger:
            try:
                ...
                if not quiet:
                    print(f"Error finishing tensorboard: {e}")
        ...
        if "clearml" in self.logger:
            try:
                ...
                if not quiet:
                    print(f"Error finishing clearml: {e}")
        
        self._finished = True

    def __del__(self):
        """Fallback cleanup using __del__, but explicit finish() is preferred."""
        # Use quiet=True to suppress messages during garbage collection
        self.finish(exit_code=0, quiet=True)
