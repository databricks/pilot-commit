...
@register_policy_loss("grpo_score")
def compute_policy_loss_grpo_score(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    # Compute score prediction loss (Binary Cross-Entropy)
    if score_preds is not None and score_targets is not None:
        # Convert score_targets to tensor if it's not already
        if not isinstance(score_targets, torch.Tensor):
            score_targets = torch.tensor(score_targets, device=score_preds.device)
        elif score_targets.device != score_preds.device:
            score_targets = score_targets.to(score_preds.device)
            
        # Convert boolean targets to float if necessary
        if score_targets.dtype == torch.bool:
            score_targets_float = score_targets.float()
        else:
            score_targets_float = score_targets.float()
        score_preds_float = score_preds.float()
        
        assert score_preds_float.shape == score_targets_float.shape, f"score_preds_float and score_targets_float have different shape: {score_preds.shape} vs. {score_targets_float.shape}"
        # Binary cross-entropy loss (score_preds is already sigmoid-ed from success_rate_head)
        # Using binary_cross_entropy (not binary_cross_entropy_with_logits) 
        # because the model already applies sigmoid in modeling_custom.py line 148
        ...
        score_loss = torch.tensor(0.0, device=advantages.device)

    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, score_loss
