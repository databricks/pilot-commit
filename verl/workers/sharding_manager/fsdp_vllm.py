...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        # Filter out custom parameters that don't exist in vLLM's model
        # (e.g., success_rate_head, reward_head, etc.)
        vllm_params = {}
        excluded_params = []
        for name, param in updated_params.items():
            # Exclude reward heads and other custom parameters
            if any(head in name for head in ['success_rate_head', 'reward_head', 'value_head']):
                excluded_params.append(name)
                continue
            vllm_params[name] = param
        
        if excluded_params:
            logger.info(f"Excluded {len(excluded_params)} custom parameters from vLLM weight loading: {excluded_params[:5]}...")
