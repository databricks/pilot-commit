...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            print(f"Actor module: {actor_module}")
            print(f"Actor module has success_rate_head: {hasattr(actor_module, 'success_rate_head')}")
            print(f"Actor module has lm_head: {hasattr(actor_module, 'lm_head')}")
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                output, entropys, scores = self.actor.compute_log_prob(data=data, calculate_entropy=False)

            if scores is not None:
                assert entropys.shape[0] == scores.shape[0], f"entropy and score have different batch size: {entropys.shape[0]} vs. {scores.shape[0]}"
                tensors = {"old_log_probs": output, "scores": scores}
            else:
